...
3. **安心できる**: 公開/非公開、権利譲渡、対価請求不可、結果非保証、受付不可カテゴリが、必要な場面で理解できる。
...
ただし初見ユーザーに対しては、抽象的な世界観より先に「ここはAllNewアプリの改善要望と新アプリ案を送る場所」と明確に伝える。トップの最初に Welcome first セクションを置き、その直下でこのサイトの目的を説明する。その後、Apple ID 連携済みユーザーに対して対話型 Step by Step ガイドを解放する。ユーザーは「アプリを選ぶ」「これでいいか確認する」「お話の種類を選ぶ」「投稿へ進む」を順番に進める。
...
1. **Task-first mobile hub**  
   初回画面では「この場所で何ができるか」と「アイデアを投稿する」「公開ボードを見る」「対象アプリを選ぶ」を優先する。PC/iPad は余白を増やすが、情報構造はスマホを正本にする。

2. **Progressive disclosure**  
   法務・プライバシー・禁止カテゴリは常時長文で見せず、投稿直前、ルール画面、モーダル、ポリシー文書に分けて提示する。表画面は前向きでわかりやすい言葉にし、重要同意は隠さず、必要な場面で必ず確認させる。

3. **Confidence before submission**  
   投稿前に、対象アプリ、公開設定、ニックネーム、受付不可カテゴリ、採用や実装時期を約束するものではないことを一目で確認できるようにする。
...
6. **Guided conversation**
   トップ画面は単なる操作一覧にせず、Apple ID 連携後に「今日はどのアプリについて、お話ししていただけますか？」から始める。アイコン選択、YES/NO確認、次ステップへの軽い移動演出で、投稿フォームに入る前の迷いをなくす。
...
1. Home
2. Apple ID 連携
3. 対話型 Step by Step ガイドで対象アプリを選ぶ
4. 「これでいいですか？ YES / NO」で確認
5. 種類: 改善要望
6. 投稿フォームへ進む
6. タイトルと説明を入力
7. 受付不可カテゴリを自己確認
8. 公開/非公開を選ぶ
9. 公開する場合は日本語/英語ニックネームをルーレットで選ぶ
10. 同意事項を確認
11. 送信
...
1. Home
2. Apple ID 連携
3. 対話型 Step by Step ガイドで New App Idea を選ぶ
4. 「これでいいですか？ YES / NO」で確認
5. 種類: 新規アプリ案
6. 投稿フォームへ進む
6. 以降は Flow A と同じ
...
|---|---|---|---|
| Home | 入口。何ができるかを即時理解し、対話型ガイドで投稿準備 | Step by Step ガイドを進める | 公開ボードを見る |
...
Home は説明ページではなく、対話型の操作ハブにする。ただし最上部だけは、初めて訪れた人が「自分に関係があるサイトだ」と分かる Welcome first セクションにする。その直下で、ユーザーが迷わず投稿準備を進められる Step by Step ガイドを開始する。

優先順位:

1. Brand and menu
2. Welcome first: あったらいいなぁ / を、カタチにする。
3. このサイトについて: AllNewアプリの声を受け取る場所であること
4. Gated Step by Step guide: Apple ID連携後にアプリ選択、YES/NO確認、投稿種別選択、投稿へ進む
5. Optional discovery: AllNew apps
6. Common items

Welcome copy:

- Main: あったらいいなぁ / を、カタチにする。
- Support: あなたのひとことが、アプリのあしたをつれてくる。
- CTA: Apple IDで続ける / 投稿をはじめる

Step by Step copy:

- Question: 今日はどのアプリについて、お話ししていただけますか？
- Confirm: これでいいですか？ YES / NO
- Next: 今日は、どんなお話ですか？
- Ready: この内容で投稿へ進む

検索は Home の一等地には置かず、公開ボードの絞り込み操作として配置する。検索は主要体験ではなく、公開済み要望が増えたときに探すための補助機能として扱う。
...
3. **同意して送る**: 権利譲渡、年齢、出願予定なし、採用や実装時期を約束するものではないこと

改善方針:

- 長い同意文は視覚的に重いので、チェック項目をカード化して読みやすくする。詳細条文は投稿ガイドラインのモーダルと法務文書へ逃がす。
