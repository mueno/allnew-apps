...
- 2026-07-03: Card image is a real app screen, never the icon. The auto-onboard/curated flows had left `card_image_path` pointing at the app icon for apps with no onboarding capture (basalsnap, meishibridge, medreminder, hikae-cards), so the card rendered a blown-up icon instead of a screenshot.
  - Card resolution is now onboarding slide -> committed `assets/asc-screenshots/<slug>.<ext>` (App Store first banner) -> empty; never the icon. Implemented in `store_discovery.resolve_card_image`, mirrored in `store_content_sync` and `update_landing_data.apply_catalog_defaults` (which self-heals any icon-as-card value on every reconcile).
  - The static renderer no longer falls back to the icon; with no real image it emits an empty grey box (matching the runtime), and the enrichment backlog flags the app.
  - The four apps' cards now use their live App Store first screenshot (JA), committed under `assets/asc-screenshots/`. The small meta-row icon is unchanged.
  - Regression guard: `tests/test_card_image_never_icon.py` asserts no shipped card image is an icon and that `resolve_card_image` never returns one.
- 2026-07-03 (2): Card image follows the display language. The four screenshot cards used the Japanese App Store banner regardless of `?lang`, so the English page showed Japanese headlines.
  - Added `card_image_path_en` / `promo_image_path_en` (catalog -> generated -> runtime). The runtime `pickCardImage(app)` returns the English variant on the English page, else falls back to the default (Japanese) card image. Language-neutral onboarding shots have no `_en` variant and serve both languages unchanged.
  - Only apps with a *genuine* English App Store screenshot get an `_en` asset: medreminder and hikae-cards do; basalsnap and meishibridge do NOT (their App Store US storefront still shows Japanese screenshots — meishibridge's `-en-US` files are Japanese content), so the English page falls back to Japanese, matching the store. Filenames were not trusted — each candidate was verified by viewing the image.
  - `store_discovery.find_store_screenshot(..., locale="en")` resolves `assets/asc-screenshots/<slug>-en.<ext>` for future auto-onboarded apps.
  - Regression guard: `tests/test_card_image_localization.py` (English assets exist and are English variants; no-English apps fall back to JA and never to an icon).
